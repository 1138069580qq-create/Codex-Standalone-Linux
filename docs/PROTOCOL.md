...
## v1.3.1 desktop saved-project identity (2026-09-08)

`project/create` alone does not register a project in the desktop saved-project cache. The attach-only bridge now opens an app-host MessageChannel and uses the desktop projects service through a bounded CapnWeb transport. Only project listing/creation and task-project assignment are exposed. Native project creation disables Git initialization and receives the already-validated existing directory. The desktop legacy ID is resolved to the app-server ID against the current backend registry and checked canonical roots; ambiguous mappings fail closed. A task is assigned to both the backend project and the desktop saved project before its first turn. Existing WebUI-created task IDs are retained during explicit project-registration repair. No raw global-state writes or arbitrary capability calls are accepted.
