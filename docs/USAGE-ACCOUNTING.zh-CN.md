...
## 2026-09-09 侧栏显示与分层反馈

侧栏恢复共享账户周窗口（仅 10080 分钟）的真实剩余百分比与重置时间；本网页账户累计费用与订阅占比继续按原公式显示。两个指标属于不同范围，不把共享周额度冒充某一个用户的专属配额。缺少窗口时显示“未获取周额度”，不推算或填零。仍使用同一份 5 / 30 分钟缓存。

设置/用量详情请求失败在其所在弹层显示，包括三级记录详情；不会只在被遮挡的首页提示。
