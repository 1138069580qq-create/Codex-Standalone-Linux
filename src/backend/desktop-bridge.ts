...
function installDesktopEventListener(name:string){
  ...
  const clip=(value:any,max=65536)=>{const text=String(value||'');let end=Math.min(text.length,max);if(end<text.length&&/[\uD800-\uDBFF]/.test(text[end-1])&&/[\uDC00-\uDFFF]/.test(text[end]))end--;return text.slice(0,end);};
  ...
  function item(value:any){if(!value||value.type==='reasoning')return null;const result:any={id:value.id,type:value.type,status:value.status};if(value.type==='userMessage')result.content=(value.content||[]).filter((c:any)=>c.type==='text').map((c:any)=>({type:'text',text:clip(c.text)}));else if(value.type==='fileChange')result.changes=(value.changes||[]).slice(0,100).map((c:any)=>({path:c.path,kind:c.kind}));else if(value.type==='commandExecution'){result.command=clip(value.command,8192);result.aggregatedOutput=clip(value.aggregatedOutput);}else result.text=clip(value.text);return result;}
  const receive=(event:MessageEvent)=>{if(event.source!==window&&event.source!==null)return;const m=event.data,p=m?.params,id=p?.threadId||p?.thread?.id;if(m?.type!=='mcp-notification'||m.hostId!=='local'||!ids.has(id)||!types.has(m.method))return;const result:any={threadId:id,method:m.method,params:{threadId:id}};const r=result.params;
    ...
    else if(m.method.startsWith('item/')){r.delta=clip(p.delta);r.itemId=p.itemId;r.turnId=p.turnId;}
