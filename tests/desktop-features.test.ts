...
test('five native main conversations across accounts; sixth has no thread or turn side effect',async()=>{
 const f=await fixture();try{const requests=Array.from({length:6},(_,i)=>f.service.createTask({uuid:'guest-'+i,elevated:false},'projectless',input('limit-request-'+i)));const results=await Promise.allSettled(requests);assert.equal(results.filter(r=>r.status==='fulfilled').length,5);assert.equal(results.filter(r=>r.status==='rejected'&&(r.reason as any).code==='CONCURRENCY_LIMIT').length,1);assert.equal(f.calls.filter(c=>c.method==='thread/start').length,5);assert.equal(f.calls.filter(c=>c.method==='turn/start').length,5);assert.equal(f.service.status(admin).maxConcurrentTurns,5);assert.equal(f.service.hasActiveWork,true);}finally{f.service.disconnect();}
});
