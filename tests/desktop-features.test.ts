...
test('slash actions use native operations, require confirmation, and never send model text for rename/pin/fork',async()=>{
  const f=await fixture();try{await assert.rejects(f.service.taskAction(admin,'demo','anchor','review',{requestId:'action-request-1',target:'uncommittedChanges'}),/确认/);for(const kind of ['rename','pin','fork','side','compact','review','feedback']){const result=await f.service.taskAction(admin,'demo','anchor',kind,{requestId:'action-request-'+kind,confirmed:true,name:'Renamed',pinned:true,target:'baseBranch',branch:'main',reason:'test feedback'});assert.equal(result.ok,true);if(['compact','review'].includes(kind)){const ipc=f.ipcs.get('anchor');ipc.state.turns=[{id:'finished-'+kind,status:'completed'}];ipc.state.threadRuntimeStatus='idle';await f.service.snapshot(admin,'demo','anchor');}}assert.ok(f.calls.some(c=>c.method==='thread/name/set'&&c.params.name==='Renamed'));assert.ok(f.calls.some(c=>c.method==='set-thread-pinned'));assert.ok(f.calls.some(c=>c.method==='thread-follower-compact-thread'));assert.ok(f.calls.some(c=>c.method==='review/start'&&c.params.delivery==='inline'));assert.ok(f.calls.some(c=>c.method==='thread/fork'&&c.params.ephemeral===true&&c.params.deferGoalContinuation===true));assert.ok(!f.calls.some(c=>c.method==='turn/start'));const feedback=f.calls.find(c=>c.method==='feedback/upload');assert.equal(feedback.params.includeLogs,false);assert.deepEqual(feedback.params.extraLogFiles,[]);assert.deepEqual((await f.service.mcp(admin,'demo','anchor')).data[0].tools,['search']);}finally{f.service.disconnect();}
});
...
test('five native main conversations across accounts; sixth has no thread or turn side effect',async()=>{
 const f=await fixture();const persistenceErrors:any[]=[];const save=(f.service as any).save.bind(f.service);(f.service as any).save=async()=>{try{return await save();}catch(error){const e=error as any;persistenceErrors.push({code:e.code,message:e.message});throw error;}};try{const requests=Array.from({length:6},(_,i)=>f.service.createTask({uuid:'guest-'+i,elevated:false},'projectless',input('limit-request-'+i)));const results=await Promise.allSettled(requests);assert.equal(results.filter(r=>r.status==='fulfilled').length,5);assert.equal(results.filter(r=>r.status==='rejected'&&(r.reason as any).code==='CONCURRENCY_LIMIT').length,1);assert.equal(f.calls.filter(c=>c.method==='thread/start').length,5);assert.equal(f.calls.filter(c=>c.method==='turn/start').length,5,JSON.stringify({results,persistenceErrors}));assert.equal(results.filter(r=>r.status==='fulfilled'&&r.value.messageAccepted===true).length,5,JSON.stringify(results));assert.equal(f.service.status(admin).maxConcurrentTurns,5);assert.equal(f.service.hasActiveWork,true);}finally{f.service.disconnect();}
});
test('regular owner can use confirmed full access, pin and fork only their own task',async()=>{
 const f=await fixture();try{const guest={uuid:'full-owner',elevated:false},r=await f.service.createTask(guest,'projectless',{...input('owner-full-access'),access:'full',confirmFullAccess:true,settingsOverrides:['access']});const turn=f.calls.find(c=>c.method==='turn/start');assert.equal(turn.params.sandboxPolicy.type,'dangerFullAccess');assert.equal(turn.params.approvalPolicy,'never');await f.service.taskAction(guest,'projectless',r.threadId,'pin',{requestId:'owner-pin-123',pinned:true});const fork=await f.service.taskAction(guest,'projectless',r.threadId,'fork',{requestId:'owner-fork-123',confirmed:true});assert.ok(fork.thread.id);await assert.rejects(f.service.snapshot({uuid:'other-user',elevated:false},'projectless',r.threadId));}finally{f.service.disconnect();}
});
test('already-running desktop mains fill the same global gate while subagent tasks do not',async()=>{
 const f=await fixture();try{for(let i=0;i<5;i++)f.threads.set('external-'+i,{id:'external-'+i,cwd:f.projectless,projectId:null,status:'running',source:'vscode'});await assert.rejects(f.service.createTask(admin,'projectless',input('external-full-123')),(e:any)=>e.code==='CONCURRENCY_LIMIT');assert.equal(f.calls.filter(c=>c.method==='thread/start').length,0);}finally{f.service.disconnect();}
});
