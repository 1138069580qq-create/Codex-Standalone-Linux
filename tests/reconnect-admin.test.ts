...
test('reset-card UI has no ordinary-member action and rechecks the account after confirmation',async()=>{
 ...
 vm.runInContext('renderQuota(limits)',context);const findButtons=(n:any):any[]=>[...(n.tag==='button'?[n]:[]),...(n.children||[]).flatMap(findButtons)];assert.equal(findButtons(nodes['quota-detail-content']).length,0);assert.doesNotMatch(JSON.stringify(nodes),/重置卡|到期|仅管理员/);
