...
test('daily pairing HTTP requires login and CSRF, protects MCP by bearer, and invalidates on logout',async t=>{
 ...
 class Fixture extends CodexConsoleService{override async createThread(_who:any,_project:string,_title?:string,_requestId?:string,prepare?:any){const p=prepare();config=p.config;return{id:'fresh-task',title:'new',status:'idle',localDeviceBinding:p.commit('fresh-task')};}override async snapshot(){return{id:'task'} as any;}override async configureLocalTools(_i:any,_p:any,_t:any,c:any){config=c;return{ok:true};}}
 ...
 const created=await post('/api/codex/threads',{projectId:'p',title:'new',requestId:'new-local-test',localDevice:{deviceId:'mac',tools}},headers);assert.equal(created.status,200);const fresh:any=await created.json();assert.equal(fresh.localDeviceBinding.threadId,'fresh-task');assert.equal(fresh.localDeviceBinding.accountId,member.id);assert.equal(fresh.localDeviceBinding.deviceId,'mac');assert.equal(JSON.stringify(fresh).includes('Bearer'),false);
 const freshToken=config['mcp_servers.local_device_files'].http_headers.Authorization;
 assert.equal((await post('/api/local-tools/mcp',{jsonrpc:'2.0',id:1,method:'tools/list'},{Authorization:freshToken})).status,200);
