...
test('reconnect keeps retrying transient session failures, stops after logout or project change',async()=>{
  const queue:Function[]=[];const state:any={epoch:1,user:{id:'qa'},project:{id:'p'},thread:{id:'a'},retry:0};const label:any={};let calls=0;let opened=0;
  const context:any={S:state,document:{hidden:false},$:()=>label,Math,controls:()=>{},clearTimeout:()=>{},setTimeout:(fn:Function)=>{queue.push(fn);return queue.length;},api:async()=>{if(++calls===1)throw new Error('offline');return {user:{id:'qa'},csrf:'test-csrf'};},syncSnapshot:()=>opened++};
...
test('upload cancels before transmission if the project changes while reading the file',async()=>{
  ...
  const context:any={uploadBusy:false,uploadCancelled:false,uploadController:null,DOMException,S:state,$:()=>upload,api:async()=>{sent++;},renderAttachments:()=>{},crypto:{subtle:{digest:async()=>new ArrayBuffer(32)}},same:(epoch:number,id:string)=>epoch===state.epoch&&id===state.project.id,Uint8Array,ArrayBuffer};
...
test('reconnect refreshes snapshots and stops forbidden or explicitly disconnected sessions',async()=>{
  for(const error of [{status:403,code:'THREAD_FORBIDDEN',message:'forbidden'},{status:503,code:'CODEX_DISCONNECTED',message:'manual disconnect'}]){
    const timers:Function[]=[],label:any={},state:any={epoch:2,user:{id:'qa'},project:{id:'p'},thread:{id:'a'},retry:0};let snapshots=0;
    const c:any={S:state,document:{hidden:false},$:()=>label,Math,clearTimeout:()=>{},setTimeout:(fn:Function)=>timers.push(fn),api:async()=>({user:{id:'qa'},csrf:'test-csrf'}),syncSnapshot:async()=>{snapshots++;throw error;},notice:()=>{},controls:()=>{}};
...
test('stale EventSource callbacks cannot change status; short-lived streams retain backoff',()=>{
  ...
  class Stream{onopen?:()=>void;onerror?:()=>void;handlers:any={};constructor(){created.push(this);}close(){}addEventListener(n:string,fn:Function){this.handlers[n]=fn;}}
  const c:any={S:state,document:{hidden:false},$:()=>label,EventSource:Stream,apiProject:()=>'',closeStream:()=>state.stream=null,clearTimeout:()=>{},setTimeout:(fn:Function)=>timers.push(fn),scheduleReconnect:()=>{},controls:()=>{},notice:()=>{},TextEncoder};
