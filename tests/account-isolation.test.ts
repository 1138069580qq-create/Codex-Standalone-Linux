...
test('HTTP account isolation: own folders and projectless chats, guessed IDs denied, statistics private, quota fetched once',async t=>{
 ...
 const receiptPath='/api/codex/threads/'+ta.data.id+'/messages/receipt-check-123?projectId=projectless';const beforeReceipt=peer.calls.filter(v=>v.method==='turn/start').length;
 assert.equal((await call(a,receiptPath)).data.state,'absent');assert.equal((await call(b,receiptPath)).status,403);assert.equal(peer.calls.filter(v=>v.method==='turn/start').length,beforeReceipt);
