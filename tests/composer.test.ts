...
function setup() {
  ...
  const S:any={historyState:'ready',user:{admin:true},connected:true,project:{id:'demo'},thread:null,threads:[],status:'idle',sending:false,epoch:1,attachments:[],selected:[],references:[],goalDraft:'',accessConfirmed:false,attempt:null};
  const calls:any[]=[]; const notices:string[]=[]; let cleared=0;const storage=new Map<string,string>();
  const c:any={module:{exports:{}},crypto:{getRandomValues:webcrypto.getRandomValues.bind(webcrypto)},Uint8Array,document:{},sessionStorage:{getItem:(k:string)=>storage.get(k)||null,setItem:(k:string,v:string)=>storage.set(k,v),removeItem:(k:string)=>storage.delete(k)},setTimeout:()=>1,clearTimeout:()=>{},confirmAction:async()=>true,el:()=>({}),loadEfforts:()=>{},S,$:(id:string)=>elements[id],permission:()=>true,stateName:(v:string)=>v,closeStream:()=>{},renderThreads:()=>{},renderAttachments:()=>{},renderSettingsSource:()=>{},notice:(v='')=>notices.push(v),clearConversation:()=>{cleared++;S.epoch++;S.thread=null;elements.prompt.value='';},syncSnapshot:async()=>{S.historyState='ready';},loadModels:async()=>{},api:async(url:string,body:any)=>{calls.push({url,body});return url.endsWith('/threads')?{id:'real-created',title:'hello',status:'idle'}:{ok:true};}};
...
test('slash menu lists all existing commands without hiding unavailable actions or including pets',()=>{
 ...
 const node=()=>({dataset:{} as Record<string,string>,children:[] as any[],append(...v:any[]){this.children.push(...v);},replaceChildren(){this.children=[];},setAttribute(){}});c.el=node;c.action=(fn:any)=>fn;elements['menu-list']=node();elements.prompt.setAttribute=()=>{};
